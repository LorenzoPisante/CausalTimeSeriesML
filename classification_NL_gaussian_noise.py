...
FILE NUOVO DEL 26 APRILE PER L'AGGIUNTA DI RUMORE AD ALGORITMI NL'
"""
...
F1_scores_per_noise_level = np.zeros((len(noise_levels), len(COUP_COEFF1)))

ROW = -1
for noise_index, noise_intensity in enumerate(noise_levels):
    for COUP_COEFF in COUP_COEFF1:
        ...
        ##Aggiungo rumore a TestData con l'intensità specificata
        testdata = testdata + np.random.normal(0, noise_intensity, testdata.shape)
        ...
        print("ChaosFEX - ACC for COUP_COEFF =  ",COUP_COEFF,"is", F1SCORE)
        
        F1_score_Result_array[ROW] = ACC 
        
        F1SCORE = accuracy_score(testlabel, y_pred_testdata)
        F1_scores_per_noise_level[noise_index, ROW % len(COUP_COEFF1)] = F1SCORE
    
    ROW=-1

# Grafico dei risultati
plt.figure(figsize=(15,10))
for i, noise_intensity in enumerate(noise_levels):
    plt.plot(COUP_COEFF1, F1_scores_per_noise_level[i], '-*', markersize=10, label=f"F1-Score at noise {noise_intensity}")
...
plt.tight_layout()
plt.savefig( rf"C:\Users\loren\Desktop\EXP_REPORT\NL\generalization\gaussian_noise_TM.png", format='png', dpi=300)
plt.show()
np.save(rf"C:\Users\loren\Desktop\EXP_REPORT\NL\generalization\gaussian_noise_TM_5", F1_scores_per_noise_level)    
