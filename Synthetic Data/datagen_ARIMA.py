# -*- coding: utf-8 -*-
"""
Created on Thu Dec 14 09:02:49 2023
...
for COEFF in COUP_COEFF:
    ...
    for NUM_TRIALS in range(0, SAMPLES_PER_CLASS):
        ...
        y[0] = y_in
        x[0] = x_in
        z[1] = a1 * z[0]  + COEFF * y[0] + e1*noise_y[1] +e2*noise_y[0]
        y[1] = c1 * y[0] + e1*noise_y[1] +e2*noise_y[0]
        x[1] = a1 * x[0]  + COEFF * y[0] + e1*noise_x[1] +e2*noise_x[0]
    
        for n in range(2, LENGTH):
            y[n] = c1*y[n - 1] + c2*y[n-2] + e1*noise_y[n]+ e2*noise_y[n-1] 
            #z[n] = a1 * z[n - 1] + a2 * z[n - 2] + e1*noise_y[n] + e2*noise_y[n-1]
            x[n]= a1 * x[n - 1] + a2 * x[n - 2] + e1*noise_x[n] + e2*noise_x[n-1] + COEFF * y[n-1]
